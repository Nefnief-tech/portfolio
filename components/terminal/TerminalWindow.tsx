...
export function TerminalWindow({ onNavigate, booted }: Props) {
  const { history, currentCommand, activeSection, isAnimating } = useTerminal();
  const bottomRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    bottomRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [{ history, currentCommand, activeSection, isAnimating }]);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto py-2">
        ...
        <OutputRenderer activeSection={activeSection} />
        <div ref={bottomRef} />
      </div>
...
export function TerminalWindow({ onNavigate, booted }: Props) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* Only show terminal interface when booted is true */}
      {booted ? (
        <div className="flex-1 overflow-y-auto py-2">
