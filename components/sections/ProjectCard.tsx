...
export function ProjectCard({ project, onExpand, delay }: Props) {
  return (
    ...
    >
      <div className="text-primary glow-primary font-bold mb-1">
        {project.name}
      </div>
      <div className="text-text-soft text-xs mb-3">{project.description}</div>
      <div className="flex flex-wrap gap-1">
        {project.tags.map((tag) => (
          <span key={tag} className="term-cyan text-xs font-mono">
            [{tag}]
          </span>
        ))}
      </div>
      <div className="flex gap-3 mt-3">
        {project.githubUrl && (
          <a
            ...
            className="term-green text-xs hover:text-primary"
        ...
        {project.liveUrl && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="term-orange text-xs hover:text-accent"
            onClick={(e) => e.stopPropagation()}
          >
            live ↗
          </a>
